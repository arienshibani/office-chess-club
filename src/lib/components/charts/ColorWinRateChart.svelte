<script>
	...
	import { colorWinRate, pieGradient } from '$lib/chess/player-stats.js';

	/** @type {{ stats: { white: { wins: number, losses: number, draws: number }, black: { wins: number, losses: number, draws: number } } }} */
	let { stats } = $props();

	/** @param {import('$lib/chess/player-stats.js').ColorRecord} record @param {'white' | 'black'} color @param {string} label */
